...
for race_no in range(1,int(no_of_race)+1):
    ...
    try:
        ...
        plt.text(0.5, 0.5, 'Copyright MonkeyForecast http://www.monkeyforecast.com', fontsize=12, color='gray', alpha=0.5, ha='center', va='center', transform=plt.gca().transAxes)

        #plt.title('Horse Relationship Diagram', pad=20)
        #plt.axis('off')
        # Save the chart as a PNG file
        plt.savefig('../racecard/static/horse_relationship'+str(race_no)+'.png')
    except:
        print("No data on this race")
    # Draw the graph
