...
race_hist=race_hist.dropna(subset=['AdjustedAge'])
...
X = horse_jockey_avg_result[['HorseName', 'Jockey','Class','Distance','Draw','Course','Venue','Going','RaceMonth','WeightRatio','RestDays','Age']]
...
for i in range(1,int(no_of_race)+1): 
    ...
    new_race_data = pd.read_csv('../racecard/data/new_current_race_'+str(i)+'.csv')
...
# Get the top 3 horses
   ...
   # Calculate score
    ...
    race_predict_df.to_csv('../racecard/data/predict_race_neu5'+str(i)+'.csv')
