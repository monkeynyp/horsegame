...
race_hist = pd.read_csv('../racecard/data/curr_hist_df_m_5.csv')
# Feature Engineering
...
#race_hist=race_hist.dropna(subset=['gear'])
...
horse_jockey_avg_result = race_hist.groupby(['HorseName', 'Jockey','Class_pur','Distance','Draw','Course','Venue','Going','RaceMonth','WeightRatio'])['Place'].mean().reset_index()
horse_jockey_avg_result.columns = ['HorseName', 'Jockey','Class','Distance','Draw','Course','Venue','Going','RaceMonth','WeightRatio','AvgResult']
...
X = horse_jockey_avg_result[['HorseName', 'Jockey','Class','Distance','Draw','Course','Venue','Going','RaceMonth','WeightRatio']]
...
# Get the top 3 horses
   ...
   # Calculate score
    ...
    race_predict_df.to_csv('../racecard/data/predict_race_neu3'+str(i)+'.csv')
    #race_prob.to_csv('../horsegame/racecard/data/predict_race_neu3_weight'+str(i)+'.csv')